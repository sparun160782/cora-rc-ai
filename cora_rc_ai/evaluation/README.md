...
### 2b. Live evaluation of the Report Agent

Set `EVAL_AGENT=report` to score the standalone report sub-agent (instead of the root
compliance orchestrator) against the report-focused evalset:

```powershell
$env:LIVE_EVAL="true"
$env:EVAL_AGENT="report"
$env:EVALSET_PATH="cora_rc_ai/evaluation/evalsets/report_agent_eval.json"
C:\Workspace\SPR-WS\cora-rc-ai\.venv\Scripts\python.exe -c "from dotenv import load_dotenv; load_dotenv('cora_rc_ai/.env'); import runpy; runpy.run_path('cora_rc_ai/evaluation/ragas_eval.py', run_name='__main__')"
```

Reset to the compliance agent afterwards in the same terminal:

```powershell
$env:EVAL_AGENT="compliance"
$env:EVALSET_PATH=""
```
...
| `EVAL_AGENT` | `compliance` | Which agent the live pipeline runs: `compliance` (root orchestrator) or `report` (report sub-agent). |
