...
class LivePipelinePopulator:
    ...
    def _resolve_agent(self):
        """Select the agent under evaluation from ``EVAL_AGENT`` config.

        ``report`` targets the standalone report sub-agent; anything else (default)
        evaluates the root compliance orchestrator.
        """
        if self._config.eval_agent == "report":
            from cora_rc_ai.backend_agentic.agents.subagents.report_agent import (  # noqa: PLC0415
                report_agent,
            )

            print("EVAL_AGENT=report — evaluating the report sub-agent.")
            return report_agent
